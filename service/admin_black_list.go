...
func JsonInBlacklist(jwtList model.JwtBlacklist) (err error) {
	err = global.MyDB.Create(&jwtList).Error
	return
}
...
func IsBlacklist(jwt string, jwtList model.JwtBlacklist) bool {
	isNotFound := global.MyDB.Where("jwt = ?", jwt).First(&jwtList).RecordNotFound()
	return !isNotFound
}
...
func GetRedisJWT(userName string) (err error, redisJWT string) {
	redisJWT, err = global.MyRedis.Get(userName).Result()
	return err, redisJWT
}
...
func SetRedisJWT(jwtList model.JwtBlacklist, userName string) (err error) {
	err = global.MyRedis.Set(userName, jwtList.Jwt, 1000*1000*1000*60*60*24*7).Err()
	return err
}
