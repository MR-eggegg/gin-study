...
func UpdateCasbinApi(oldPath string, newPath string, oldMethod string, newMethod string) error {
	...
	err := global.MyDB.Table("casbin_rule").Where("v1 = ? AND v2 = ?", oldPath, oldMethod).Find(&cs).Updates(map[string]string{
		"v1": newPath,
		"v2": newMethod,
	}).Error
	return err
...
func Casbin() *casbin.Enforcer {
	a := gormadapter.NewAdapterByDB(global.MyDB)
	e := casbin.NewEnforcer(global.MyConfig.Casbin.ModelPath, a)
